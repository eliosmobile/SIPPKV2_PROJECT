        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header bg-primary text-white">
                    <h4 class="card-title">{{ __('Approval Requests') }}</h4>
                            ...
                            <tbody>
                                @foreach ($requests as $request)
                                    <tr>
                                        ...
                                        <td>
                                            <form action="{{ route('wadir.requests.approve', $request->id) }}" method="POST" style="display:inline-block;">
                                                @csrf
                                                <button type="submit" class="btn btn-success btn-sm">
                                                    <i class="fas fa-check"></i> {{ __('Approve') }}
                                                </button>
                                            </form>
                                            <form action="{{ route('wadir.requests.reject', $request->id) }}" method="POST" style="display:inline-block;">
                                                @csrf
